...
(function() {
  var Notification, Promise, Transaction, bodyParser, config, connect, crypto, http, util, xmlObj;
  ...
  Notification = class Notification {
    constructor() {
      ...
      this.callbacks = {};
      this.listener = config.notifications;
    }
    ...
    */
    verifySignature(params) {
      ...
      switch (params.signature.length) {
        case 40:
          hash = crypto.createHash('sha1').update(unique_id + config.customer.password).digest('hex');
          break;
        case 64:
          hash = crypto.createHash('sha256').update(unique_id + config.customer.password).digest('hex');
          break;
        case 128:
          hash = crypto.createHash('sha512').update(unique_id + config.customer.password).digest('hex');
          break;
        default:
          hash = new String();
      }
