...
Transaction =
  require path.resolve './src/genesis/transactions/non_financial/kyc/verification/kyc_verification_register'

describe 'KYC Verification Register Request', ->
  ...
  context 'when valid request', ->
    ...
    it 'with proper endpoint url', ->
      assert.equal @transaction.getUrl().path, 'api/v1/verifications/register'
