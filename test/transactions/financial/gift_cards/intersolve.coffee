...
Transaction                   =
  require path.resolve './src/genesis/transactions/financial/gift_cards/intersolve'

describe 'Intersolve Transaction', ->
  ...
  it 'when remember_card with invalid value', ->
    @data.cvv = 'no'

    assert.isNotTrue @transaction.setData(@data).isValid()

  it 'when consumer_id with invalid long value', ->
    @data.consumer_id = '12345678901'
